...
def create_app():
    ...
    def process_login():
        ...
        if l_form.validate_on_submit(): #делаем проверку формы: если пришли данные формы, и пришли без ошибок, в том числе без отсутствия необходимых данных
            user=User.query.filter(User.login==l_form.login.data).first()
            if user and user.check_password(l_form.password.data): #если пользователь существует и пароль верный
                login_user(user) #залогинить пользователя. здесь происходит внутренняя магия flask_login'а
...
def test_db ():
    user_test=User(name='Vasya', surname='Ivanov', email='b@b.ru', login='VIb', password='password', gender='Male', role='User')
    print (user_test)
